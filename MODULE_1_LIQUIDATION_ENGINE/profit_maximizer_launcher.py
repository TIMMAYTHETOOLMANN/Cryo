...
# Safe way to get current directory that works in all contexts
try:
...
class ProfitMaximizer:
    ...
    def _launch_strategy(self, name, config):
        ...
        try:
            ...
            if name == "liquidation_storm":
                result = module.start(**params)
            elif name == "arbitrage_network":
                result = module.execute_arbitrage(**params)
            elif name == "flash_loan_assault":
                result = module.execute_flash_loan(**params)
            elif name == "mev_hunter":
                result = module.execute_backrun(**params)
            else:
                print(f"❌ Unknown strategy: {name}")
                return False
            
            # Standardize return value: extract success if it's a dict
    ...
    def launch_full_spectrum(self):
        ...
        with ThreadPoolExecutor(max_workers=len(strategies)) as executor:
            ...
            # Count successful strategies (all results are now boolean)
            successful_count = sum(1 for result in results if result)
            
            print(f"🚀 All strategies launched successfully: {successful_count}/{len(results)} successful")
